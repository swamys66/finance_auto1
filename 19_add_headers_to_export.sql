...
-- Step 2: Create JavaScript stored procedure (simpler string handling)
...
AS
$$
    // Step 1: Load data from existing file into temp table
    var loadSql = `CREATE OR REPLACE TEMPORARY TABLE temp_export_data AS 
    ...
    snowflake.execute({sqlText: loadSql});
    
    // Step 2: Create header split table (already exists from Step 1, but ensure it's there)
    var headerSql = `CREATE OR REPLACE TEMPORARY TABLE temp_header_split AS
        SELECT 
    ...
    snowflake.execute({sqlText: headerSql});
    
    // Step 3: Export header + data
    var exportSql = `COPY INTO @dev_data_ingress.finance.s3_test_finance_automation_output/${NEW_FILE_NAME}
    ...
    snowflake.execute({sqlText: exportSql});
